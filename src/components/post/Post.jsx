  ...
  const likeHandler = async () => {
    try {
      if (isLiked) {
        ...
        await axios.post(`${API_URL}${API_BASE}/posts/${post.id}/like`);
        setLike(like + 1);
      }
      setIsLiked(!isLiked);
    } catch (error) {
    ...
    <div className="bg-white/80 backdrop-blur-xl rounded-2xl shadow-xl shadow-gray-500/5 border border-white/20 p-6 hover:shadow-2xl hover:shadow-blue-500/10 transform hover:-translate-y-1 transition-all duration-500 group">
      <div className="flex space-x-4">
        ...
        <div className="flex-1">
          ...
          <p className="text-gray-800 mb-4 leading-relaxed font-medium">{post.content}</p>
          {post.image && <img className="w-full max-h-96 object-cover rounded-xl" src={post.image} alt="" />}
