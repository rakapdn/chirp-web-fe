...
export default function OtherUserProfile() {
    ...
    const fetchProfile = async () => {
      ...
      try {
        ...
        if (error.response?.status === 401) {
          ...
          setErrorMessage(
            error.response?.data?.error || "Gagal memuat profil. Coba lagi nanti."
          );
        }
      } finally {
  ...
  useEffect(() => {
    const fetchPosts = async () => {
      ...
      try {
        ...
        setPostsError(
          error.response?.data?.error || "Gagal memuat postingan."
        );
      } finally {
  ...
  const handleFollow = async () => {
    ...
    try {
      ...
      setErrorMessage(
        error.response?.data?.error || "Gagal mengikuti pengguna. Coba lagi nanti."
      );
    } finally {
  ...
  const handleUnfollow = async () => {
    ...
    try {
      ...
      setErrorMessage(
        error.response?.data?.error || "Gagal berhenti mengikuti pengguna. Coba lagi nanti."
      );
    } finally {
      setFollowLoading(false);
    }
          ...
          <div className="bg-white/80 backdrop-blur-xl rounded-2xl shadow-xl shadow-blue-500/10 border border-white/20 p-6 mb-6">
            <div className="flex items-center space-x-6">
              <div className="w-24 h-24 rounded-full bg-gradient-to-r from-blue-600 to-purple-600 flex items-center justify-center text-white font-bold text-3xl shadow-lg overflow-hidden">
                {profileData?.image ? (
                  <img
                    src={`${API_URL}${API_BASE}/images/${encodeURIComponent(profileData.image)}`}
              ...
              <div className="flex-1">
                <h1 className="text-2xl font-bold text-gray-800">{profileData?.username || "Unknown"}</h1>
                <p className="text-gray-600">{profileData?.fullName || "Nama Lengkap"}</p>
              ...
              >
                {followLoading ? "Memproses..." : isFollowing ? "Berhenti Mengikuti" : "Ikuti"}
              </button>
            </div>
          </div>
          {/* Postingan Pengguna */}
